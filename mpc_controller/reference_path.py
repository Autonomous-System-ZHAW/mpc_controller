...
class ReferencePath:
    ...
    def _construct_path(self, wp_x, wp_y):
        ...
        wp_ys = []
        for wp_id in range(
            self.smoothing_distance, len(wp_x) - self.smoothing_distance
        ):
            wp_xs.append(
                np.mean(
                    wp_x[
                        wp_id
                        - self.smoothing_distance : wp_id
    ...
    def _compute_length(self):
        ...
        """
        segment_lengths = [0.0] + [
            self.waypoints[wp_id + 1] - self.waypoints[wp_id]
            for wp_id in range(len(self.waypoints) - 1)
        ]
    ...
    def _compute_width(self, max_width):
        ...
        for wp_id, wp in enumerate(self.waypoints):
            ...
            for i, dir in enumerate(["left", "right"]):
                ...
                if dir == "left":
                    angle = np.mod(wp.psi + np.pi / 2 + np.pi, 2 * np.pi) - np.pi
                else:
                    angle = np.mod(wp.psi - np.pi / 2 + np.pi, 2 * np.pi) - np.pi
    ...
    def _compute_free_segments(self, wp, min_width):
        ...
        for x, y in zip(x_list[1:], y_list[1:]):
            ...
            if (self.map.data[y, x] == 0 or (x, y) == lb_p) and free_cells:
                ...
                if (
                    np.sqrt((ub_o[0] - lb_o[0]) ** 2 + (ub_o[1] - lb_o[1]) ** 2)
                    > min_width
                ):
    ...
    def update_path_constraints(self, wp_id, N, min_width, safety_margin):
        ...
        for n in range(N):
            ...
            if n == 0:
                segment_lengths = [
                    np.sqrt((seg[0][0] - seg[1][0]) ** 2 + (seg[0][1] - seg[1][1]) ** 2)
                ...
                if len(free_segments) >= 2:
                    ...
                    for free_segment in free_segments:
                        ...
                        d_ub = np.sqrt(
                            (ub_fs[0] - ub_pw[0]) ** 2 + (ub_fs[1] - ub_pw[1]) ** 2
                        )
                        d_lb = np.sqrt(
                            (lb_fs[0] - lb_pw[0]) ** 2 + (lb_fs[1] - lb_pw[1]) ** 2
                        )
            ...
            angle_ub = (
                np.mod(
                    np.arctan2(ub_ls[1] - wp.y, ub_ls[0] - wp.x) - wp.psi + np.pi,
                    2 * np.pi,
                )
                - np.pi
            )
            angle_lb = (
                np.mod(
                    np.arctan2(lb_ls[1] - wp.y, lb_ls[0] - wp.x) - wp.psi + np.pi,
                    2 * np.pi,
                )
                - np.pi
            )
            ...
            # Compute cell on bound for computed distance ub and lb
            ub_ls = wp.x + (ub + safety_margin) * np.cos(angle_ub), wp.y + (
                ub + safety_margin
            ) * np.sin(angle_ub)
            lb_ls = wp.x - (lb - safety_margin) * np.cos(angle_lb), wp.y - (
                lb - safety_margin
            ) * np.sin(angle_lb)
