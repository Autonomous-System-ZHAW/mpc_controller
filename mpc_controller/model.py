...
class simple_bycicle_model:
    ...
    def model_setup(self):
        ...
        # lateral (orthogonal) distance from vehicle position to reference path
        ...
        # introduce expression psi difference for monitoring
        self.model.set_expression("psi_diff", psi_diff)

        self.model.set_rhs("pos_x", vel * np.cos(psi))
        self.model.set_rhs("pos_y", vel * np.sin(psi))
        # going with exact value: np.sin(delta) instead of just delta
        self.model.set_rhs("psi", vel * np.sin(delta) / self.length)
        self.model.set_rhs("vel", acc)
        # same as top but with the diff of psi
        self.model.set_rhs("e_y", vel * np.sin(psi_diff))
